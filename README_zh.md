...
**3. 现有 Wasm 分子库体积过大**
RDKit.js（C++ RDKit 的 Wasm 移植版）功能丰富，但 **Bundle 大小超过 ~10MB**。OpenChemLib（基于 Java，~3MB）在 2D 描绘和 SMILES 处理上表现出色，但不支持 PDB / 3D 空间查询。对于只需要距离查询和近邻搜索的应用来说，两者都超出所需。
...
| 离线可用 | 是 | 是 | 是 | 否 | 是 |
| 支持 10k+ 原子 | 是 | 是 | 否 | 是 | 否 |
| Bundle 大小 | 小 (~200KB) | 大(~10MB+) | 中(~3MB) | — | 小 |
| 不阻塞 UI | 是 (Web Worker) | 部分支持 | 部分支持 | — | 否 |
| 无需安装 | 是 | 是 | 是 | 否 | 是 |
| C/C++ 依赖 | 无 | 有 | 无 | 有 | 无 |
